...
class Pipeline(Workflow):
    ...
    async def arun(self) -> AsyncIterator[RunResponse]:
        ...
        with open(output_path, "w") as f:
            json.dump(aggregated_data.model_dump(), f, indent=4)
        ...
        identities = (
            IdentityList.model_validate(self.session_state["identities"])
            if "identities" in self.session_state
            else None
        )
        if not identities:
            log.info("Invoking Identity_Agent...")
            identity_query = queries["identity_query"].format(
                aggregated_data_json=aggregated_data.model_dump_json(indent=2)
            )
...
if __name__ == "__main__":
    ...
    async def _main():
        ...
        # Stoage persistance problem: the memory is not being persisted within the SqliteStorage after the session completes..
