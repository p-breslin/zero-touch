...
MAX_REPOS = int(os.getenv("MAX_REPOS_TO_SNAPSHOT", 500))  # avoid API rate limit
...
def _collect_file_tree(org: str, repo_name: str, branch: str = None) -> List[Tuple]:
    try:
...
def _get_org_repos(org_name: str) -> List[Tuple[str, str]]:
    try:
...
def snapshot_org_repos(org_name: str):
    ...
    log.info("Discovered %d repositories in org %s", len(repo_targets), org_name)

    for org, repo in repo_targets:
        rows = _collect_file_tree(org, repo)
